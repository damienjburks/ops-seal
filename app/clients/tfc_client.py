...
# class TfcClient:
#     def __init__(self, token, api_url="https://app.terraform.io/api/v2"):
#         """
...
#         logging.info(f"Retrieved {len(workspaces)} workspaces for organization: {org_name}")
#         return workspaces
...
#         logging.info(f"Checking if the most recent apply for workspace ID '{workspace_id}' was a destroy.")
...
#             logging.warning(f"No completed apply runs found for workspace ID '{workspace_id}'.")
#             return False

#         except requests.RequestException as e:
#             logging.error(f"Failed to check the most recent apply for workspace ID '{workspace_id}': {e}")
#             logging.debug(traceback.format_exc())
#             return False
...
#                 logging.info(f"Auto Apply successfully enabled for workspace ID: {workspace_id}")
#                 return True
#             else:
#                 logging.error(f"Failed to enable Auto Apply for workspace ID '{workspace_id}': {res.text}")
#                 return False
#         except requests.RequestException as e:
#             logging.error(f"Error enabling Auto Apply for workspace ID '{workspace_id}': {e}")
#             logging.debug(traceback.format_exc())
#             return False
...
#         logging.info(f"Creating destroy run for workspace: {workspace_name} (ID: {workspace_id})")
#         try:
#             res = requests.post(url, headers=self.headers, json=payload)
#             if res.status_code == 201:
#                 run_id = res.json()["data"]["id"]
#                 logging.info(f"[✓] Destroy run created for workspace '{workspace_name}' (run_id: {run_id})")
#             else:
#                 logging.error(f"[x] Failed to create destroy run for '{workspace_name}': {res.text}")
#         except requests.RequestException as e:
#             logging.error(f"Failed to create destroy run for workspace '{workspace_name}': {e}")
#             logging.debug(traceback.format_exc())
...
#                 logging.info(f"  ⏩ Skipping workspace '{ws_name}' (whitelisted in {org_name})")
#                 continue
...
#                     logging.info(f"    [!] Last apply for workspace '{ws_name}' was a destroy. Skipping...")
#                     continue
...
#         """
#         logging.info("🚀 Starting TFC Client script")
#         try:
...
#             logging.debug(traceback.format_exc())
#         logging.info("🏁 TFC Client script finished")
...
#     # Initialize and run the TfcClient
#     tfc_client = TfcClient(token=os.environ["TFC_TOKEN"])
#     tfc_client.run()
